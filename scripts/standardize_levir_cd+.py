...
def standardize_levir_cd(levir_root, out_root):
    sets = ['train', 'test']

    for split in sets:
        print(f"\n[→] Processing split: {split}")

        split_dir = os.path.join(levir_root, split)
        ...
        a_images = sorted([f for f in os.listdir(a_dir) if f.endswith(('.png', '.jpg'))])
...
if __name__ == "__main__":
    standardize_levir_cd(
        levir_root=r"data\LEVIR-CD+",
        out_root=r"data\standardized\levir_cd_plus"
    )
